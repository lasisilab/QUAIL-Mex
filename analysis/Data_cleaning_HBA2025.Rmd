...
# merged dataset
df <- read.csv("../data/01.SCREENING.csv", stringsAsFactors = FALSE,  na.strings = c("", "N/A", "NA", "pending"))
...
## PSS: some questions should be coded backwards: 4 (very often) equals 0, 3 (fairly often) equals 1, 2 (sometimes) equals 2, 1 (almost never) equals 3, 0 (never) equals 4). This coding should apply to all the positive statements (Q4, 5, 6, 7, 9, 10 and 13)

# Identify columns that start with "SES" and end with "SC"
pss_columns <- grep("^PSS*", names(merged_df), value = TRUE)

columns_to_transform <- c("PSS4", "PSS5", "PSS6", "PSS7", "PSS9", "PSS10", "PSS13")
# Convert 
merged_df <- merged_df %>%
  mutate(across(all_of(columns_to_transform), ~ case_when(
    . == 4 ~ 0,
...
merged_df$PSS_TOTAL_REV <- rowSums(merged_df[(pss_columns)]) 
