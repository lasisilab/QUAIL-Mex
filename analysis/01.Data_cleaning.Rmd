...
```{r apply-resolutions}
resolved <- read.csv(file.path(data_path, "conflicts_reviewed.csv"),
                     stringsAsFactors = FALSE)
...
# Uncomment once ready to export:
 write.csv(conflicts, file.path(data_path, "conflicts_to_review_V2.csv"),
           row.names = FALSE)
...
```{r}
resolved <- read.csv(file.path(data_path, "conflicts_reviewed_V2.csv"),
                     stringsAsFactors = FALSE)
  ...
  mutate(
    HLTH_SMK_CLEAN = case_when(
      ...
      str_trim(tolower(HLTH_SMK)) %in% c(
        ...
        "casi nada", "poquito- 3 x semana", "3 x semana", "3 al mes",
