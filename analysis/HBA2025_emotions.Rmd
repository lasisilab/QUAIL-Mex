...
```
# Transform comparision to rest of the city
```{r}
data <- data %>%
  mutate(MX28_WQ_COMP = case_when(
    MX28_WQ_COMP %in% c(1, 2) ~ 0, ## same or better recoded as zero
    MX28_WQ_COMP == 0 ~ 1, # worse recoded as 1
    TRUE ~ MX28_WQ_COMP
  ))

data$MX28_WQ_COMP <- factor(data$MX28_WQ_COMP, levels = c("0", "1"))
```
...
data <- data %>%
  ...
  rename(PSS_TOTAL = PSS_TOTAL_REV)
...
odds_df$Predictor <- factor(odds_df$Predictor, 
                                  levels = c("(Intercept)", "HW_TOTAL", "PSS_TOTAL", "HRS_WEEK", "MX28_WQ_COMP1"),
                                  labels = c("Intercept",
                                             "HWISE\nscore", 
                                             "Perceived\nStress", 
                                             "Hours of\nWater Supply", 
