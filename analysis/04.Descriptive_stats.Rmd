...
```{r hwise-pss-overall}
tbl_hwise_pss_overall <- df_table %>%
  select(HWISE_Total, HWISE_Cat, Water_worry, Water_5sub, PSS_Total, PSS_Cat) %>%
  tbl_summary(
    ...
    label   = list(
      ...
      Water_5sub   ~ "Water 5item subscale (0-15)",
...
tbl_hwise_pss_overall
```

## 6.1.2 HWISE only

```{r hwise-overall}
tbl_hwise_overall <- df_table %>%
  select(HWISE_Total, HWISE_Cat, Water_worry, Water_5sub) %>%
  tbl_summary(
    type = list(Water_worry ~ "continuous"),
    statistic = list(
      all_continuous()  ~ "{mean} ({sd}) / {median} [{p25}, {p75}]",
  ...
  bold_labels() %>%
  modify_caption("**Table. HWISE scores — full sample (n = {N})**")
...
## 6.2.1 SEASON HWISE only



```{r hwise-by-season}
tbl_hw_scores_season <- df_table %>%
  select(Season, HWISE_Total, HWISE_Cat, Water_worry, Water_5sub) %>%
  tbl_summary(
    by        = Season,
    type = list(Water_worry ~ "continuous"),
    statistic = list(
  ...
  bold_labels() %>%
  modify_spanning_header(c("stat_1", "stat_2") ~ "**Season**") %>%
  modify_caption("**Table. HWISE by season (n = {N})**")

tbl_hw_scores_season
```
...
item scores and totals (ordinal); Fisher's exact test for categories.
...
tbl_hwise_nbhd <- df_table %>%
  ...
  modify_footnote(
    ...
    p.value ~ "Kruskal-Wallis for item scores/totals; Fisher's exact test for categories.",
...
tbl_hwise_season <- df_table %>%
  ...
  modify_footnote(
    all_stat_cols() ~ "Median (Q1, Q3) for continuous; n (%) for categorical.",
    p.value ~ "Wilcoxon rank-sum for item scores/totals; Fisher's exact test for categories.",
    q.value ~ "FDR-adjusted p-value (Benjamini-Hochberg)."
  )
...
## 8.2 All water-related variables (overall and by season)

This table consolidates all water-related variables in one place:
HWISE items (1-12), HWISE summary scores/categories, and objective
water supply characteristics. Presented overall and stratified by season.

```{r water-all-by-season}
tbl_water_all <- df_water %>%
  mutate(
  ...
  select(
    Season,
    HWISE_Total, HWISE_Cat, Water_worry, Water_5sub,
    HW_Worry, HW_Interr, HW_Clothes, HW_Plans,
    HW_Food, HW_Hands, HW_Body, HW_Drink,
    HW_Angry, HW_Sleep, HW_None, HW_Shame,
    W_WS_LOC_F, W_Source, W_Continuity, W_Frequency,
  ...
  bold_labels() %>%
  modify_spanning_header(c("stat_1", "stat_2") ~ "**By season**") %>%
  modify_caption("**Table. All water-related variables (overall and by season) (n = {N})**")

tbl_water_all
```

## 8.3 Summary table by neighborhood
...
## 8.4 Sentinel code audit for water supply variables
...
tables <- list(
  ...
  "TableS2_HWISE_PSS_overall.docx"         = tbl_hwise_pss_overall,
  "TableS2.1_HWISE_overall.docx"           = tbl_hwise_overall,
  "TableS3_HWISE_by_season.docx"           = tbl_hw_scores_season,
  "TableS4_characteristics_by_HWISE.docx"  = tbl_by_hwise,
  "TableS5_water_supply_by_neighborhood.docx" = tbl_water,
  "TableS6_all_water_variables_by_season.docx" = tbl_water_all,
  "TableS7_HWISE_items_by_neighborhood.docx" = tbl_hwise_nbhd,
  "TableS8_HWISE_items_by_season.docx"      = tbl_hwise_season
)
