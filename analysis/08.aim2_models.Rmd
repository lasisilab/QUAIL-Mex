...
| Augmenting (one per model) | HWISE individual items · Water worry subscale (`HW_WORRY_SUB`) · Hours of water supply/week · Intermittent supply · Season · Neighborhood comparison · Excessive time handling water · Sufficiency rating · Water pressure · Supply frequency · Neighborhood (WS/WI) · Years in neighborhood (`D_LOC_TIME`) |
...
term_labels <- c(
  ...
  W_FrequencyDaily               = "Daily (ref: Continuous)",
  `W_Frequency4–6 days/week`     = "4\u20136 days/wk (ref: Continuous)",
  `W_Frequency1–3 days/week`     = "1\u20133 days/wk (ref: Continuous)",
  W_FrequencyIntermittent        = "Intermittent (ref: Continuous)",
...
model_specs_pss <- c(
  ...
  list(
    ...
    m_a_loc = list(
      label   = "m_a_loc: Years in neighborhood",
...
tbl_sum_pss %>%
  select(label, term, n, AIC, beta_ci,
...
ft_pss <- top5_pss %>%
  ...
  merge_v(j = c("N", "AIC", "p_adj")) %>%
  valign(j = c("N", "AIC", "p_adj"),
...
model_specs_emo <- c(
  ...
  list(
    ...
    m_b_loc = list(
      label   = "m_b_loc: Years in neighborhood",
...
tbl_sum_emo %>%
  select(label, term, n, AIC, or_ci,
         p_coef_s, LRT_chi2, p_lrt_s) %>%
  flextable() %>%
  set_header_labels(
    label     = "Model",
    term      = "Predictor term",
