...
```{r}
# Load the dataset
file_path <- "../data/Q8-26-28.csv"
df <- read.csv(file_path, stringsAsFactors = FALSE, na.strings = c("", "N/A", "NA", "pending"))
df2 <- read.csv("./data/Cleaned_Dataset_Screening_HWISE_PSS_V2.csv")

# Ensure HW_TOTAL is numeric
df2$HW_TOTAL <- as.numeric(df2$HW_TOTAL)

# Convert all character columns to NA where values match missing patterns
df <- df %>%
  select(ID, MX28_WQ_COMP) %>%
  mutate(across(where(is.character), ~ ifelse(. %in% c("", "N/A", "NA", "pending"), NA, .)))
...
dim(merged_df)
# Save the merged dataset
write.csv(merged_df, "../data/Cleaned_Dataset_Screening_HWISE_PSS_V3.csv", row.names = FALSE)
