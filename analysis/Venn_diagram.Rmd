...
overlap1 <- sum(sapply(split_areas, function(x) all(c("quality", "pressure") %in% x)))
overlap <- sum(sapply(split_areas, function(x) all(c("quality", "quantity") %in% x)))
...
# Plot
# Plot the diagram
plot(venn_diagram, fills = c("skyblue", "pink", "orange"), edges = TRUE, labels = TRUE)
