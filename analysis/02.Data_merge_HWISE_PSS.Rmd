...
This script merges the cleaned screening file (`Screening_clean.csv`, produced by
...
cat("IDs only in screening:        ", length(only_screen), "\n")
cat("IDs only in HWISE_PSS:        ", length(only_hw),     "\n")
...
hwise_pss %>% filter(is.na(PSS_TOTAL)) %>% select(ID, all_of(PSS_ITEMS), PSS_INFO)
...
```{r save}
out_file <- file.path(data_path, "Merged_Screening_HWISE_PSS.csv")
