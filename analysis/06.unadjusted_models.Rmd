...
predictor_order <- c(
  ...
  W_Days_week = 5, W_Hrs_week = 6, W_Pressure = 7, Yrs_in_nbhd = 8
)
...
# Helper: unadjusted linear regression
...
# Helper: unadjusted logistic regression
...
```{r uvreg-export}
header_border <- fp_border(color = "#2c3e50", width = 1.5)
...
cat("Compact tables built; forest plot tables will be appended in unadj-forest-tables-export.\n")
```
...
uv_group_order <- c("Demographics", "Socioeconomic", "Health",
                    "HWISE", "HWISE items", "Water supply")

uv_predictor_order <- c(
  Age = 1, Parity = 2, HH_size = 3,
  SES_Score = 1, Edu_Score = 2, Employed = 3, Employment_type = 4,
  Chronic_pain = 1, Chronic_dis = 2, PSS_Total = 3,
  HWISE_Total = 1, Water_5sub = 2, HWI_5sub_hi = 3, HWISE_Cat = 4, Water_worry = 5,
...
```{r unadj-forest-data}
# ── Load & prepare data for forest plot tables ─────────────────────────────
MISSING_CODES_UV    <- c(999, 888, 777, 666, 555, 444, 333)
SENTINEL_STRINGS_UV <- as.character(MISSING_CODES_UV)

clean_for_table_uv <- function(x) {
  x_num <- suppressWarnings(as.numeric(as.character(x)))
  ifelse(x_num %in% MISSING_CODES_UV | is.na(x_num), NA, x_num)
}
is_sentinel_str_uv <- function(x) stringr::str_trim(as.character(x)) %in% SENTINEL_STRINGS_UV

df_raw_uv <- read.csv("./data/03.Merged_Q1-6_Q16_Q19_Q26_Q28.csv",
...
df_table_uv <- df_raw_uv %>%
  mutate(
    Age         = clean_for_table_uv(D_AGE),
    Yrs_in_nbhd = clean_for_table_uv(D_LOC_TIME_CLEAN),
    HH_size     = clean_for_table_uv(D_HH_SIZE),
    Parity      = clean_for_table_uv(D_CHLD_CLEAN),
    Season      = factor(SEASON, levels = c(0, 1), labels = c("Rainy", "Dry")),
    Education = factor(
      dplyr::recode(as.character(SES_EDU_CAT_CLEAN),
        "Incomplete primary"     = "Incomplete primary (1–5 yrs)",
      ...
      levels = edu_levels_uv, ordered = TRUE
    ),
    SES_Score       = clean_for_table_uv(SES_SC_Total),
    Edu_Score       = as.numeric(Education),
    Employed        = factor(clean_for_table_uv(D_EMP_CLEAN),
                             levels = c(0, 1), labels = c("No", "Yes")),
    Employment_type = factor(clean_for_table_uv(D_EMP_FORM_CLEAN),
                             levels = c(0, 1), labels = c("Informal", "Formal")),
    Chronic_pain    = factor(clean_for_table_uv(HLTH_CPAIN_CAT),
                             levels = c(0, 1), labels = c("No", "Yes")),
    Chronic_dis     = factor(clean_for_table_uv(HLTH_CDIS_CAT),
                             levels = c(0, 1), labels = c("No", "Yes")),
    HWISE_Total = clean_for_table_uv(HW_TOTAL),
    Water_worry = clean_for_table_uv(as.numeric(HW_WORRY_SUB)),
    Water_5sub  = clean_for_table_uv(HW_MEX_5SUB),
    HWISE_Cat   = factor(
      case_when(
        is.na(clean_for_table_uv(HW_TOTAL))  ~ NA_character_,
        clean_for_table_uv(HW_TOTAL) <= 3    ~ "Marginal (0-3)",
        clean_for_table_uv(HW_TOTAL) <= 11   ~ "Low (4-11)",
        TRUE                                  ~ "Moderate/Severe (\u226512)"
      ),
      levels = c("Marginal (0-3)", "Low (4-11)", "Moderate/Severe (\u226512)")
    ),
    ...
    HW_Worry   = clean_for_table_uv(HW_WORRY),
    HW_Interr  = clean_for_table_uv(HW_INTERR),
    HW_Clothes = clean_for_table_uv(HW_CLOTHES),
    HW_Plans   = clean_for_table_uv(HW_PLANS),
    HW_Food    = clean_for_table_uv(HW_FOOD),
    HW_Hands   = clean_for_table_uv(HW_HANDS),
    HW_Body    = clean_for_table_uv(HW_BODY),
    HW_Drink   = clean_for_table_uv(HW_DRINK),
    HW_Angry   = clean_for_table_uv(HW_ANGRY),
    HW_Sleep   = clean_for_table_uv(HW_SLEEP),
    HW_None    = clean_for_table_uv(HW_NONE),
    HW_Shame   = clean_for_table_uv(HW_SHAME),
    PSS_Total  = clean_for_table_uv(PSS_TOTAL)
  )

df_water_uv <- df_raw_uv %>%
  mutate(
    W_Continuity = case_when(
      is_sentinel_str_uv(MX3_CONT_INT)                             ~ NA_character_,
      stringr::str_detect(tolower(MX3_CONT_INT), "^contin")        ~ "Continuous",
      stringr::str_detect(tolower(MX3_CONT_INT), "^intermi|cortan|deja") ~ "Intermittent",
      stringr::str_detect(tolower(MX3_CONT_INT), "disminu")        ~ "Reduced pressure/flow",
      MX3_CONT_INT %in% c("", "N/A") | is.na(MX3_CONT_INT)       ~ NA_character_,
      TRUE                                                          ~ "Other"
    ),
    W_Continuity = factor(W_Continuity,
                          levels = c("Continuous", "Intermittent",
                                     "Reduced pressure/flow", "Other")),
    W_Freq_raw  = dplyr::coalesce(MX4_WFREQ, MX4_W_FREQ),
    W_Frequency = case_when(
      is_sentinel_str_uv(W_Freq_raw)                               ~ NA_character_,
      stringr::str_detect(tolower(W_Freq_raw), "^diari|every day") ~ "Daily",
      stringr::str_detect(tolower(W_Freq_raw), "^intermi")         ~ "Intermittent",
      stringr::str_detect(tolower(W_Freq_raw), "1 a 3|1-3")        ~ "1\u20133 days/week",
      stringr::str_detect(tolower(W_Freq_raw), "4 a 6|4-6")        ~ "4\u20136 days/week",
      stringr::str_detect(tolower(W_Freq_raw), "^contin")          ~ "Continuous",
      W_Freq_raw %in% c("", "N/A") | is.na(W_Freq_raw)           ~ NA_character_,
      TRUE                                                          ~ "Other"
    ),
    W_Frequency = factor(W_Frequency,
                         levels = c("Continuous", "Daily", "4\u20136 days/week",
                                    "1\u20133 days/week", "Intermittent", "Other")),
    W_Hrs_week  = suppressWarnings(
      ifelse(is_sentinel_str_uv(MX5_WFREQ_HRS_PER_WEEK), NA,
             as.numeric(stringr::str_extract(MX5_WFREQ_HRS_PER_WEEK, "^[0-9\\.]+")))),
    W_Hrs_week  = ifelse(W_Hrs_week %in% MISSING_CODES_UV | W_Hrs_week > 168, NA, W_Hrs_week),
    W_Days_week = suppressWarnings(
      ifelse(is_sentinel_str_uv(MX5_WFREQ_NDAYS), NA,
             as.numeric(stringr::str_extract(MX5_WFREQ_NDAYS, "^[0-9]+")))),
    W_Days_week = ifelse(W_Days_week %in% MISSING_CODES_UV | W_Days_week > 7, NA, W_Days_week),
    W_Pressure  = case_when(
      is_sentinel_str_uv(MX6_WPRESS)                                     ~ NA_character_,
      stringr::str_detect(tolower(MX6_WPRESS), "^s[i\u00ed]|^yes|^buena|^bien") ~ "Yes",
      stringr::str_detect(tolower(MX6_WPRESS), "^no|^baja|^poca")        ~ "No/Low",
      MX6_WPRESS %in% c("", "N/A") | is.na(MX6_WPRESS)                 ~ NA_character_,
      TRUE                                                                 ~ "Variable"
    ),
    W_Pressure  = factor(W_Pressure, levels = c("Yes", "No/Low", "Variable")),
...
EMO_raw_uv <- suppressWarnings(as.numeric(as.character(df_raw_uv$MX26_EM_HHW_CAT)))
EMO_raw_uv <- ifelse(EMO_raw_uv %in% MISSING_CODES_UV, NA, EMO_raw_uv)

df_reg_uv <- df_table_uv %>%
  mutate(
    EMO_NEG      = case_when(EMO_raw_uv == 0 ~ 0L, EMO_raw_uv == 1 ~ 1L, TRUE ~ NA_integer_),
    W_WS_LOC_F   = df_water_uv$W_WS_LOC_F,
    W_Continuity = df_water_uv$W_Continuity,
    W_Frequency  = df_water_uv$W_Frequency,
    W_Days_week  = df_water_uv$W_Days_week,
    W_Hrs_week   = df_water_uv$W_Hrs_week,
    W_Pressure   = df_water_uv$W_Pressure
  )

ref_levels_uv <- sapply(names(df_reg_uv), function(v) {
  x <- df_reg_uv[[v]]
  if (is.factor(x)) levels(x)[1] else NA_character_
})

make_display_label_uv <- function(predictor, term, label) {
  if (predictor == term) return(label)
  level <- gsub(predictor, "", term, fixed = TRUE)
  ref   <- ref_levels_uv[predictor]
...
# ── Regression runners ────────────────────────────────────────────────────────
run_lm_uv_fp <- function(outcome, predictors, data, labels) {
...
run_glm_uv_fp <- function(outcome, predictors, data, labels) {
  purrr::map_dfr(predictors, function(v) {
    dat <- data[, c(outcome, v)]
    for (drop_lvl in c("Variable", "Other")) {
      if (v %in% c("W_Pressure", "W_Frequency", "W_Continuity") &&
...
pss_results_fp <- run_lm_uv_fp("PSS_Total", pss_predictors_fp, df_reg_uv, uv_pred_labels) %>%
  mutate(
    q.value       = p.adjust(p.value, method = "BH"),
    group         = group_map[predictor],
    group         = factor(group, levels = uv_group_order),
    var_order     = uv_predictor_order[predictor],
    display_label = mapply(make_display_label_uv, predictor, term, label),
...
emo_results_fp <- run_glm_uv_fp("EMO_NEG", emo_predictors_fp, df_reg_uv, uv_pred_labels) %>%
  mutate(
    q.value       = p.adjust(p.value, method = "BH"),
    group         = group_map[predictor],
    group         = factor(group, levels = uv_group_order),
