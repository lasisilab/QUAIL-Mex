...
- **Multiple testing**: Significance thresholds and row highlighting in all
  tables and plots are based on **Benjamini–Hochberg (BH) adjusted *q*-values**,
  applied across all predictors within each outcome.
...
pss_tbl <- pss_results %>%
  ...
  select(display_label, n, `β (95% CI)`, p_fmt, sig, AIC_r, p.value) %>%
  rename(
...
pss_gt <- pss_tbl %>%
  ...
  tab_style(
    ...
    locations = cells_body(columns = c(N, `p-value`, Sig, AIC))
  ) %>%
  tab_style(
    style     = cell_text(align = "center"),
    locations = cells_column_labels(columns = c(N, `p-value`, Sig, AIC))
  ) %>%
  cols_width(
    Variable       ~ px(210),
    N              ~ px(50),
...
emo_tbl <- emo_results %>%
  ...
  select(display_label, n, `OR (95% CI)`, p_fmt, sig, AIC_r, p.value) %>%
  rename(
    Variable       = display_label,
    N              = n,
    `p-value`      = p_fmt,
