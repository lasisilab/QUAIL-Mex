...
reg_dataset = merged_dataset[c(1, 2, 4:7, 9:11, 38:42)]
reg_dataset$PSS_TOTAL = rowSums(merged_dataset[(23 + c(1,2,3,8,11,12,14))]) - rowSums(merged_dataset[(23 + c(4,5,6,7,9,10,13))])

create_na_table(reg_dataset)
...
HW_lm = lm(HW_TOTAL ~ MX28_WQ_COMP+ SEASON+W_WS_LOC+W_WC_WI+HRS_WEEK+D_CHLD+SES_SC_Total, data = reg_dataset)
summary(HW_lm)
```
...
PSS_lm = lm(PSS_TOTAL ~ D_LOC_TIME+MX28_WQ_COMP+SEASON+W_WS_LOC+W_WC_WI+HRS_WEEK+D_AGE+D_HH_SIZE+D_CHLD+SES_SC_Total+HW_TOTAL, data = reg_dataset)
summary(PSS_lm)
```
