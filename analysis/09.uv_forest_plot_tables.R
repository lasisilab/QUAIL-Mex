# 09. Univariable Forest Plot Tables
# Creates publication-ready tables from 07.univariate_models forest plot results
# (PSS linear + Emotion logistic) saved into one Word document.
# Run from project root: Rscript analysis/09.uv_forest_plot_tables.R
...
group_map <- c(
  ...
  Chronic_pain = "Health", Chronic_dis = "Health", PSS_Total = "Health",
  HWISE_Total = "HWISE", Water_5sub = "HWISE",
  HWI_5sub_hi = "HWISE", HWISE_Cat = "HWISE", Water_worry = "HWISE",
...
# ── Load data ─────────────────────────────────────────────────────────────────
df_raw <- read.csv("./data/03.Merged_Q1-6_Q16_Q19_Q26_Q28.csv",
...
EMO_raw <- suppressWarnings(as.numeric(as.character(df_raw$MX26_EM_HHW_CAT)))
EMO_raw <- ifelse(EMO_raw %in% MISSING_CODES, NA, EMO_raw)

df_reg <- df_table %>%
  mutate(
    EMO_NEG      = case_when(EMO_raw == 0 ~ 0L, EMO_raw == 1 ~ 1L, TRUE ~ NA_integer_),
...
run_lm_uv <- function(outcome, predictors, data, labels) {
  map_dfr(predictors, function(v) {
    ...
    fit <- tryCatch(lm(as.formula(paste(outcome, "~ .")), data = dat), error = function(e) NULL)
...
run_glm_uv <- function(outcome, predictors, data, labels) {
  map_dfr(predictors, function(v) {
    ...
    dat <- dat[complete.cases(dat), ]
    fit <- tryCatch(glm(as.formula(paste(outcome, "~ .")), data = dat, family = binomial),
...
cat("Fitting PSS univariable models...\n")
...
cat("Fitting Emotion univariable models...\n")
...
# Insert shaded group header rows
insert_group_headers <- function(df, group_col = "Group") {
...
build_uv_ft <- function(results, outcome_label, est_col_label,
                         footnote_txt, is_logistic = FALSE) {
  ...
  tbl_with_headers <- insert_group_headers(tbl_data) %>%
    select(-Group)
  ...
  sig_rows  <- which(!data$q_fmt %in% c("—", NA) & is_sig(data$q_fmt))
  nsig_rows <- setdiff(seq_len(nrow(data)), sig_rows)
  sig_coef  <- which(!data$p_fmt %in% c("—", NA) &
                       suppressWarnings(as.numeric(gsub("<", "0", data$p_fmt))) < 0.05)
  ...
  ft <- data %>%
    ...
    # ── Column header ────────────────────────────────────────────────────────
    bg(bg = COL_HEADER, i = 2, part = "header") %>%
    color(color = "white", i = 2, part = "header") %>%
    ...
    # ── Alignment ────────────────────────────────────────────────────────────
    align(j = c("N", "AIC", "p_fmt", "q_fmt"), align = "center", part = "all") %>%
    align(j = "est_ci", align = "right", part = "body") %>%
    # ── Font ─────────────────────────────────────────────────────────────────
...
# ── Footnotes ─────────────────────────────────────────────────────────────────
pss_footnote <- paste0(
  "Each predictor entered in a separate univariable linear regression model. ",
  ...
  "HWISE (total score, 5-item subscale variants, HWISE categorical, Water worry subscale); ",
...
emo_footnote <- paste0(
  "Each predictor entered in a separate univariable logistic regression model. ",
...
# ── Build tables ──────────────────────────────────────────────────────────────
cat("Building PSS table...\n")
ft_pss <- build_uv_ft(
  results        = pss_results,
  outcome_label  = "Table UV1. Univariable Linear Regression \u2014 Perceived Stress Score (PSS)",
  est_col_label  = "\u03b2 [95% CI]",
  footnote_txt   = pss_footnote,
  is_logistic    = FALSE
)

cat("Building Emotion table...\n")
ft_emo <- build_uv_ft(
  results        = emo_results,
  outcome_label  = "Table UV2. Univariable Logistic Regression \u2014 Negative Emotional Response to Water",
  est_col_label  = "OR [95% CI]",
  footnote_txt   = emo_footnote,
  is_logistic    = TRUE
)

# ── Save to Word ──────────────────────────────────────────────────────────────
portrait_sec <- prop_section(
...
doc <- read_docx() %>%
  ...
  body_set_default_section(portrait_sec)

outpath <- "./output/tables/Tables_UV_ForestPlot_Results.docx"
